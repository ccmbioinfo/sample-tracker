...
const actionSelectOptions = {
    "add2Cohort": "Move to a different Cohort",
    "updateAnalysisStatus": "Update Analysis status",
    "updateSolvedStatus": "Update Solved status",
    "requestReanalysis": "Request reanalysis",
    "assignTo": "Assign to user"
};

const columns = [
    {text: 'SampleID', dataField: 'SampleID', hidden: true}, {
        text: 'DatasetID',
        dataField: 'datasetID',
        hidden: true
    }, {text: 'Cohort', dataField: 'activeCohort', sort: true, filter: textFilter()}, {
        text: 'Family code',
        dataField: 'FamilyID',
        sort: true,
        filter: textFilter()
    }, {
        text: 'Participant code', dataField: 'Sample', sort: true, filter: textFilter(), formatter: (cell, row) => {
            return (<SampleLink sampledetails={row}/>);
        }
    }, {text: 'Dataset type', dataField: 'datasetType', sort: true, filter: textFilter()}, {
        text: 'Date entered',
        ...
        dataField: 'AnalysisStatus',
        sort: true,
        filter: textFilter(),
        formatter: (cell, row) => {
            return (<AnalysisPopOver sampledetails={row}/>);
        }
    }, {
        text: 'Analysis status updated on ',
        dataField: 'AnalysisDate',
        sort: true,
        filter: textFilter()
    }, {text: 'Solved status', dataField: 'Status', sort: true, filter: textFilter()}, {
        text: 'Assigned to',
        dataField: 'AssignedTo',
        sort: true,
        filter: textFilter()
    }, {text: 'Notes', dataField: 'Notes', sort: false, filter: textFilter()}
];
    ...
    render() {
        ...
        return (
            <div>
                    ...
                    <Col md={2}>
                        <FormGroup controlId='actionSelect_bottom' bsSize="sm">
                            ...
                            <FormControl bsSize="sm" componentClass="select" placeholder="select"
                                         onChange={this.handleActionSelect} value={this.state.actionSelectValue}>
                                <option value=""></option>
                                {Object.keys(actionSelectOptions).map(opt => {
                                    if (opt == 'assignTo') {
                                        if (this.state.userAccessLevel == 'Admin') {
                                            return (
                                                <option key={opt} value={opt}> {actionSelectOptions[opt]} </option>);
                                        }
                                    } else {
                ...
                <ToolkitProvider data={this.props.samples} columns={columns} keyField='datasetID' exportCSV>
                    {
                        props => (
                            <div>
                                <BootstrapTable noDataIndication={this.props.noSampleStr} {...props.baseProps}
                                                selectRow={selectRow} pagination={customPagination}
                ...
                <ActionModal action={this.state.actionSelectValue} actionSelectOptions={actionSelectOptions}
                             selectedSamples={this.state.selectedSamples}
                             sampleTableReset={this.resetSampleTableState}/>}
            </div>
        );
