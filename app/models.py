...
class Sample(db.Model):
	...
	FamilyID = db.Column(db.String(30), db.ForeignKey('family.FamilyID', onupdate="cascade", ondelete="cascade"),
						 nullable=False)
...
class Cohort(db.Model):
	...
	ProjectID = db.Column(db.Integer, db.ForeignKey('project.ProjectID', onupdate="cascade", ondelete="restrict"))
	Obsolete = db.Column(db.Enum(ObsoleteStatus), default=ObsoleteStatus.No.value,
						 server_default=ObsoleteStatus.No.value)
...
class Dataset(db.Model):
	__table_args__ = (
		db.UniqueConstraint('SampleID', 'EnteredDate', 'DatasetType',
							name='unique_sample_datasettype_per_entered_date'),
	)
	DatasetID = db.Column(db.Integer, primary_key=True)
	SampleID = db.Column(db.String(50), db.ForeignKey('sample.SampleID', onupdate="cascade", ondelete="restrict"),
						 nullable=False)
	EnteredDate = db.Column(db.Date, nullable=True)
	EnteredBy = db.Column(db.Integer, db.ForeignKey('uploaders.UploadID', onupdate="cascade", ondelete="restrict"),
						  nullable=True)
	...
	NotesLastUpdatedBy = db.Column(db.Integer, db.ForeignKey('user.id', onupdate="cascade", ondelete="restrict"),
								   nullable=True)
...
class Dataset2Uploaders(db.Model):
	...
	DatasetID = db.Column(db.Integer, db.ForeignKey('dataset.DatasetID', onupdate="cascade", ondelete="restrict"),
						  primary_key=True, nullable=True)
	UploadDate = db.Column(db.Date, nullable=True)
	UploaderID = db.Column(db.Integer, db.ForeignKey('uploaders.UploadID', onupdate="cascade", ondelete="restrict"),
						   nullable=True)
	UploadStatus = db.Column(db.String(45), nullable=False, default="pending", server_default="pending")
...
class Dataset2Cohort(db.Model):
	...
	DatasetID = db.Column(db.Integer, db.ForeignKey('dataset.DatasetID', onupdate="cascade", ondelete="cascade"),
...
class AnalysisStatus(db.Model):
	...
	AnalysisID = db.Column(db.Integer, db.ForeignKey('analysis.AnalysisID', onupdate="cascade", ondelete="cascade"),
						   nullable=False, primary_key=True)
